...
class OlxJobSource:
    """Public OLX job-search adapter using the site's public search pages."""

    _base_url = "https://www.olx.ua/uk/rabota/q-"
    _origin = "https://www.olx.ua"
    _user_agent = "Mozilla/5.0 (compatible; PidrobitokBot/1.0)"

    @property
    def name(self) -> str:
        return JobSource.OLX.value
    ...
    async def search(
        ...
        *,
        ...
        headers = {"User-Agent": self._user_agent, "Accept-Language": "uk-UA,uk;q=0.9"}
        ...
        for anchor in soup.select("a[href]"):
            ...
            if not isinstance(href, str) or not title:
                continue
            if not self._looks_like_advert_url(href):
                continue

            url = urljoin(self._origin, href).split("?", 1)[0]
            if url in seen_urls or len(title) > 180:
                continue
    ...
    @staticmethod
    def _looks_like_advert_url(href: str) -> bool:
        return "/d/" in href and "obyavlenie" in href
    ...
    def _extract_salary(text: str) -> tuple[Decimal | None, Decimal | None, str | None]:
        ...
        minimum = OlxJobSource._decimal(first)
        maximum = OlxJobSource._decimal(second) if second else minimum
