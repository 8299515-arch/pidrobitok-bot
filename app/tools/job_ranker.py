...
class JobRanker:
    ...
    def rank(
        ...
        return sorted(
            ranked,
            key=lambda item: (-item.score, item.job.title.casefold()),
        )
    ...
    def _query_score(
        ...
        if query.skills:
            ...
            if matches:
                matched += 1
                reasons.append(
                    f"Совпадают навыки: {', '.join(matches[:5])}"
                )
    ...
    def _candidate_score(
        ...
        if profile.skills:
            ...
            if any(
                skill.casefold() in (job.title or "").casefold()
                for skill in profile.skills
            ):
        ...
        return (
            round((weighted_match / weighted_total) * 100)
            if weighted_total
            else None
        )

    @staticmethod
    def _matches_profile_requirements(
        job: Job,
        profile: CandidateProfile,
    ) -> bool:
        if profile.city:
            if (
                not job.city
                or (
                    profile.city.casefold() not in job.city.casefold()
                    and job.remote is not True
                )
            ):
                return False
        ...
        if (
            ...
            and not JobRanker._salary_matches(job, profile.salary_min)
        ):
            return False
    ...
    def _candidate_reasons(
        ...
        if matches:
            reasons.append(
                f"Профиль: навыки {', '.join(matches[:5])}"
            )

        if (
            profile.city
            and job.city
            and profile.city.casefold() in job.city.casefold()
        ):
        ...
        if (
            profile.salary_min is not None
            and JobRanker._salary_matches(job, profile.salary_min)
        ):
    ...
    def _salary_matches(
        ...
        return (
            advertised_upper is not None
            and advertised_upper >= minimum
        )